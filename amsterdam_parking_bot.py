...
class AmsterdamParkingBot:
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        if config_file.exists():
            ...
            logging.warning(
                f"Created config file: {config_path} - Please update credentials"
            )
    ...
    def _parse_time_range(
        self, time_range: str, target_date: Optional[datetime] = None
    ...
    def _calculate_sessions(
        ...
        while current_time < end_time:
            session_end = min(
                current_time + timedelta(minutes=session_minutes), end_time
            )
    ...
    def _book_single_session(
        self, session: ParkingSession, target_date: Optional[datetime] = None
    ) -> bool:
        ...
        try:
            ...
            self.driver.get(
                "https://aanmeldenparkeren.amsterdam.nl/parking-sessions/new"
            )
            self.wait.until(
                EC.presence_of_element_located(
                    (By.CSS_SELECTOR, "input[type='time'], select")
                )
            )
                ...
                try:
                    date_select = self.wait.until(
                        EC.element_to_be_clickable(
                            (By.CSS_SELECTOR, "select[name='startDay']")
                        )
                    )
            ...
            try:
                start_time_input = self.wait.until(
                    EC.element_to_be_clickable(
                        (By.CSS_SELECTOR, "input[name='startTimeRaw']")
                    )
                )
                ...
                logging.debug(
                    f"Converting {session.start_time.strftime('%H:%M')} to {start_time_12h}"
                )
            ...
            try:
                end_time_input = self.wait.until(
                    EC.element_to_be_clickable(
                        (By.CSS_SELECTOR, "input[name='endTimeRaw']")
                    )
                )
                ...
                logging.debug(
                    f"Converting {session.end_time.strftime('%H:%M')} to {end_time_12h}"
                )
            ...
            try:
                kenteken_button = self.wait.until(
                    EC.element_to_be_clickable(
            ...
            self.wait.until(
                EC.presence_of_element_located(
                    (By.XPATH, "//input[@type='radio'][@value='MQR108']")
                )
            )
            ...
            try:
                confirm_button = self.wait.until(
                    EC.element_to_be_clickable(
                        (
                            By.XPATH,
                            "//button[contains(text(), 'Bevestig parkeersessie')]",
                        )
                    )
                )
            ...
            try:
                WebDriverWait(self.driver, 15).until(
                    EC.any_of(
                        ...
                        EC.presence_of_element_located(
                            (By.XPATH, "//*[contains(text(), 'bevestigd')]")
                        ),
                        EC.presence_of_element_located(
                            (By.XPATH, "//*[contains(text(), 'gepland')]")
                        ),
                    )
                )
    ...
    def book_parking_sessions(
        self,
        time_range: str,
        session_minutes: int = None,
        max_break_minutes: int = None,
        ...
        try:
            ...
            sessions = self._calculate_sessions(
                start_time, end_time, session_minutes, max_break_minutes
            )

            date_str = (
                "today" if booking_date is None else booking_date.strftime("%A, %B %d")
            )
            logging.info(
                f"Booking {len(sessions)} sessions for {time_range} on {date_str}"
            )
...
def main():
    ...
    parser.add_argument(
        "--tomorrow", action="store_const", const="tomorrow", dest="target_date"
    )
